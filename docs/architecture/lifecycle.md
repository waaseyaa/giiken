...
- Compilation flows traverse pipeline steps and persist knowledge items via repository. `CompilationPipeline::compile()` returns the populated `CompilationPayload` (populated `entityUuid`, `knowledgeType`, `accessTier`) so callers — `App\Console\IngestFileCommand::run()` for the `giiken:ingest:file` CLI, the Management UI tomorrow — can surface the persisted id without a follow-up query (waaseyaa/giiken#95). The CLI entrypoint is a thin Symfony `Command` registered in `AppServiceProvider::commands()`; all ingest orchestration and MIME detection live in `IngestFileCommand` with zero Symfony imports, using `IngestFileCliWriterInterface` for stdout/stderr lines. Optional `$accessTier`, `$forcedType`, and `$dryRun` parameters let operators override persisted access tier, skip `ClassifyStep`'s LLM round-trip, and run all steps without touching the repository or embedding store.
