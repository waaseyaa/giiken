...
- `SearchService::hybridSearch` tokenizes multi-word queries before hitting FTS to work around `Waaseyaa\Search\Fts5SearchProvider::escapeQuery`, which quotes each term and hands them to FTS5 MATCH as an implicit AND. The service drops English stopwords and trivially short tokens, then issues one FTS `SearchRequest` per surviving term and merges the per-term hits by keeping each doc's best raw score. Queries that tokenize to zero terms (pure stopwords) fall back to a single-shot pass of the original string so the vendor escaper sees exactly what it would have seen pre-tokenization.
